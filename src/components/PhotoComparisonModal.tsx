...
import { Award, ArrowRight, CheckCircle2, Sliders } from 'lucide-react';
  ...
  return (
    <Dialog open={!!photoA && !!photoB} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto bg-slate-900 border-slate-800 text-white p-4 md:p-6">
        ...
        <div className="bg-slate-800/80 border border-slate-700/80 rounded-xl p-3 flex items-center justify-between my-2 text-xs">
          <div className="flex items-center gap-2">
            <Award className="w-5 h-5 text-amber-400" />
            <span className="font-semibold text-slate-200">Quality Score Delta:</span>
          </div>
          <div className="font-mono text-sm font-bold">
            {scoreDiff > 0 ? (
              <span className="text-emerald-400">+{scoreDiff} pts Improvement! 🚀</span>
            ) : scoreDiff < 0 ? (
              <span className="text-rose-400">{scoreDiff} pts lower</span>
            ) : (
              <span className="text-slate-400">Identical scores</span>
            )}
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 my-3">
          ...
          <div className="bg-slate-950 border border-slate-800 rounded-xl p-3 space-y-3">
            <div className="flex justify-between items-center text-xs text-slate-400">
              <span className="font-semibold text-slate-200">Shot A (Earlier)</span>
              <span className="font-mono">{new Date(photoA.timestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
            </div>
            <div className="aspect-[4/3] rounded-lg overflow-hidden border border-slate-800 bg-black flex items-center justify-center">
            ...
            <div className="space-y-1.5 text-xs">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-mono">{photoA.analysis.compositionScore}%</span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-400">Lighting & Contrast:</span>
                <span className="font-mono">{photoA.analysis.lightingScore}%</span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-400">Guide Used:</span>
                <span className="font-mono capitalize text-emerald-400">{photoA.analysis.guideUsed.replace('_', ' ')}</span>
              </div>
            </div>
          </div>
          ...
          <div className="bg-slate-950 border border-slate-800 rounded-xl p-3 space-y-3">
            <div className="flex justify-between items-center text-xs text-slate-400">
              <span className="font-semibold text-slate-200">Shot B (Later)</span>
            ...
            <div className="space-y-1.5 text-xs">
              <div className="flex justify-between">
                <span className="text-slate-400">Overall Rating:</span>
                <span className="font-mono font-bold text-emerald-400">{photoB.analysis.overallScore}/100</span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-400">Composition:</span>
                <span className="font-mono">{photoB.analysis.compositionScore}%</span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-400">Lighting & Contrast:</span>
                <span className="font-mono">{photoB.analysis.lightingScore}%</span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-400">Guide Used:</span>
                <span className="font-mono capitalize text-emerald-400">{photoB.analysis.guideUsed.replace('_', ' ')}</span>
              </div>
            </div>
          </div>
        </div>
