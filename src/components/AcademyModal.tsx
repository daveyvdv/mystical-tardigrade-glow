...
import { CompositionGuideType, PhotographyMode, PhotoLesson } from '../types/camera';
import { Dialog, DialogContent, DialogHeader, DialogTitle, DialogDescription } from '@/components/ui/dialog';
import { Button } from '@/components/ui/button';
import { BookOpen, Check, Target, Lightbulb, Play } from 'lucide-react';
...
export const AcademyModal: React.FC<AcademyModalProps> = ({ isOpen, onClose, onApplyLesson }) => {
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="max-w-4xl max-h-[85vh] overflow-y-auto bg-slate-900 border-slate-800 text-white p-4 md:p-6">
        <DialogHeader>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 my-3">
          {PHOTO_LESSONS.map((lesson) => (
            <div
              key={lesson.id}
              className="bg-slate-800/60 border border-slate-700/80 rounded-xl overflow-hidden hover:border-emerald-500/50 transition-all flex flex-col justify-between"
            >
              <div className="relative h-36 bg-slate-950 overflow-hidden">
                ...
                />
                <div className="absolute top-2 left-2 bg-slate-900/90 text-amber-300 text-[10px] font-semibold px-2 py-0.5 rounded-full border border-amber-500/30 uppercase tracking-wide">
                  {lesson.category}
                </div>
              </div>
              ...
              <div className="p-4 flex-1 flex flex-col justify-between space-y-3">
                <div>
                  <h3 className="font-bold text-slate-100 text-base mb-1">{lesson.title}</h3>
                  ...
                  <div className="space-y-1">
                    ...
                    <ul className="text-xs text-slate-300 space-y-1 list-disc list-inside pl-1">
                ...
                <Button
                  ...
                  className="w-full bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-semibold mt-2"
                >
