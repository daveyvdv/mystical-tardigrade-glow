    ...
    <div className="absolute inset-0 pointer-events-none z-10 overflow-hidden">
      ...
      {guide === 'thirds' && (
        <div className="w-full h-full relative">
          ...
          <div className="absolute top-3 left-3 bg-black/60 backdrop-blur-md px-2 py-1 rounded text-[11px] text-amber-300 font-medium border border-amber-500/30">
      ...
      {guide === 'golden_spiral' && (
        <svg className="w-full h-full stroke-amber-400/80 fill-none" viewBox="0 0 100 100" preserveAspectRatio="none">
          ...
          <path
            ...
            strokeWidth="0.8"
            className="stroke-amber-400 drop-shadow-[0_0_8px_rgba(251,191,36,0.5)]"
          />
          <circle cx="70.8" cy="76.4" r="2" className="fill-amber-400 stroke-none animate-ping" />
        </svg>
      )}

      {/* 3. Leading Lines / Perspective */}
      {guide === 'leading_lines' && (
        <svg className="w-full h-full stroke-cyan-400/80 fill-none" viewBox="0 0 100 100" preserveAspectRatio="none">
          ...
          {/* Focal Target */}
          <circle cx="50" cy="40" r="4" strokeWidth="0.8" className="stroke-cyan-300 animate-pulse" />
      ...
      {guide === 'center' && (
        <div className="w-full h-full relative flex items-center justify-center">
          <div className="absolute inset-x-0 top-1/2 h-[1px] bg-emerald-400/60" />
          <div className="absolute inset-y-0 left-1/2 w-[1px] bg-emerald-400/60" />
          
          <div className="w-24 h-24 border border-emerald-400/50 rounded-full flex items-center justify-center">
            <div className="w-12 h-12 border-2 border-emerald-400/90 rounded-full flex items-center justify-center">
      ...
      {guide === 'framing' && (
        <div className="w-full h-full relative p-8 md:p-14">
          <div className="w-full h-full border-2 border-purple-400/80 rounded-2xl border-dashed flex flex-col justify-between p-4">
            <div className="flex justify-between text-xs text-purple-300 font-mono">
              <span>┌ FRAME TOP</span>
              <span>RIGHT ┐</span>
            </div>
            <div className="text-center text-xs text-purple-200/80 bg-black/40 py-1 px-3 rounded-full self-center">
      ...
      >
        <div className={`h-3 w-8 rounded-l border-y-2 border-l-2 ${Math.abs(tiltAngle) < 2 ? 'border-emerald-400 bg-emerald-500/20' : 'border-amber-400 bg-amber-500/20'}`} />
        <div className={`flex-1 h-[2px] mx-2 ${Math.abs(tiltAngle) < 2 ? 'bg-emerald-400 shadow-[0_0_8px_#34d399]' : 'bg-amber-400'}`} />
        <div className={`h-3 w-8 rounded-r border-y-2 border-r-2 ${Math.abs(tiltAngle) < 2 ? 'border-emerald-400 bg-emerald-500/20' : 'border-amber-400 bg-amber-500/20'}`} />
      </div>
    </div>
  );
};