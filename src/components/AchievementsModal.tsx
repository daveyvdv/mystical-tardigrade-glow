...
export const AchievementsModal: React.FC<AchievementsModalProps> = ({ isOpen, onClose, achievements }) => {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="max-w-2xl max-h-[85vh] overflow-y-auto bg-zinc-950 border-amber-500/30 text-white p-4 md:p-6">
        <DialogHeader>
          ...
          {achievements.map((a) => (
            ...
            >
              <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${
                a.unlocked ? 'bg-amber-400 text-zinc-950 font-bold' : 'bg-zinc-800 text-zinc-500'
              }`}>
              ...
              <div className="flex-1 space-y-1">
                ...
                {!a.unlocked && (
                  <div className="pt-1">
                    <Progress value={a.progress} className="h-1.5 bg-zinc-800" />
                  </div>
                )}
              </div>
            </div>
